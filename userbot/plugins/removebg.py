...
async def remove_background(event):
    ...
    if event.reply_to_msg_id and not input_str:
        ...
        userevent = await edit_or_reply(event, "`Analysing...`")
        file_name = os.path.join(TEMP_DIR, "rmbg.png")
        try:
            await event.client.download_media(reply_message, file_name)
        except Exception as e:
            await edit_or_reply(userevent, f"`{str(e)}`")
            return
        else:
            await userevent.edit("`Removing Background of this media`")
            file_name = convert_toimage(file_name)
            response = ReTrieveFile(file_name)
            os.remove(file_name)
    elif input_str:
        userevent = await edit_or_reply(event, "`Removing Background of this media`")
    ...
    if "image" in contentType:
        ...
        await edit_or_reply(userevent, f"`{response.content.decode('UTF-8')}`")
        return
    if cmd == "srmbg":
