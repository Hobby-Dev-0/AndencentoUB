...
# ========================= CONSTANTS ============================
UNAPPROVED_MSG = (
    "`Hello! This is pmpermit of Andencento userbot.\n\n`"
...
async def permitpm(event):
    ...
    if PM_AUTO_BAN:
        ...
        if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
            ...
            try:
                from userbot.sql.pmpermit_sql import is_approved
                from userbot.sql.globals import gvarstatus
...
async def auto_accept(event):
    ...
    if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
        ...
        try:
            from userbot.sql.pmpermit_sql import is_approved
            from userbot.sql.pmpermit_sql import approve
        except AttributeError:
            return
